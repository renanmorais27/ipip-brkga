...
IPIPDecoder::IPIPDecoder(const IPIPInstance& _instance) : instance(_instance) {}

IPIPDecoder::~IPIPDecoder() {}

 double IPIPDecoder::decode(const std::vector<double> &chromosome) const {
    // 1) Solve the problem (i.e., compute the power indices out of this chromosome):
	...
	IPIPSolver solver(instance, chromosome);

	// 2) Extract the fitness (delta error):
	const double fitness = solver.getDeltaError();
